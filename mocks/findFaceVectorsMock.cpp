...
{
	ros::init(argc, argv, "find_face_vector_mock");

	ros::NodeHandle n;
	ros::ServiceClient client = n.serviceClient<human_vision_exchange::FindFaceVectors>("/human_dlib_face_recognition/findFaceVectors");
    ...
    thirdImage = readImage("face2.png");
...
std::string getPathToImage(std::string imageName)
{
	...
	pathToImage = ros::package::getPath("human_dlib_face_recognition") + "/images/" + imageName;
...
std::vector< std::vector<double>> callFindFaceVector(ros::ServiceClient& client, std::vector<cv_bridge::CvImage>& faces)
{
	...
	human_vision_exchange::FindFaceVectors callMsg;
