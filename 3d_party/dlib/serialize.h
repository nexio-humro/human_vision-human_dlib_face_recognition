    ...
    public:
        explicit proxy_deserialize (
            const std::string& filename_
             ...
             fin_optional_owning_ptr(new std::ifstream(filename_.c_str(), std::ios::binary)),
             fin(*fin_optional_owning_ptr)   
        {
            if (!fin)
            {
				std::cout<<std::endl<<"proxy_deserialize::proxy_deserialize = "<<filename<<std::endl;
                throw serialization_error("throw: Unable to open " + filename + " for reading, fin_optional_owning_ptr->good() = " + static_cast<char>(fin_optional_owning_ptr->good()) );
			}
            init();
        }
